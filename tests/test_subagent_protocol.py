...
class SubagentProtocolTests(unittest.TestCase):
    ...
    def test_midstage_gates_precede_expensive_work(self):
        ...
        self.assertLess(writing.index("执行 `W1`"), writing.index("真实编译 PDF"))
    ...
    def test_latex_only_delivery_and_missing_subagent_have_explicit_states(self):
        root = read("SKILL.md")
        protocol = read("references/Subagent调度.md")
        writing = read("references/roles/论文手/SKILL.md")
        self.assertIn("受限交付", root)
        self.assertIn("受限交付", protocol)
        self.assertIn("完整论文-LaTeX/", root)
        self.assertIn("完整论文.pdf", protocol)
        self.assertIn("不创建或转换 Word/DOCX", writing)
        self.assertIn("完整论文.build.json", writing)
