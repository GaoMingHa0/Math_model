...
4. 在全量计算、参数扫描和正式出图前，按 `../../../references/验证与运行治理.md` 完成最小端到端样例、基线比较和资源预算，并派发独立质检 Subagent 执行 `P1` 最小可运行结果门禁；实现问题由编程手修正，模型合同问题携证据返回建模手。未返回 `PASS` 不得继续扩展。
...
8. 对 L2/L3 完成收敛或多种子检查、约束余量、基线对照和独立交叉验证；长运行或失败运行保留阶段状态、检查点和错误信息。生成复现清单：`python scripts/repro_manifest.py --project-root <PROJECT_ROOT> ...`。实际读取关键规则时将路径和 SHA-256 保存到现有执行记录；复现清单可用已有 `--input` 收录相关规则文件。比较读取时与验收时版本，实质变化则复核受影响主张，不把末次哈希冒充当时读取版本。
9. 按 `references/质检清单.md` 完成作者自检，再派发独立质检 Subagent 执行 `P2` 编程终检；未返回 `PASS` 不得进入论文阶段或宣称编程交付完成。

## 阶段内独立门禁

- `P1`：质检 Subagent 在隔离环境或只读副本中执行最小命令，核对退出码、输入到结果的追溯、单位、数值范围、关键约束、验证等级、子问题合同和基线样例。它是纵向切片，不要求完整图表或最终性能。
- `P2`：代码、结果、每类至少 3 张且覆盖全部子问题的三类图和复现清单冻结后，质检 Subagent 独立运行唯一复现命令并核对输入哈希、种子、关键数值、边界、量纲、收敛/鲁棒性证据、约束余量、运行记录、各子问题图表语义及文件完整性。机械图审继续使用带全部 `--questions` 的 `figure_audit.py --strict`，Subagent 负责实际读图，检查单图核心结论、主次面板、图例、统计区间和最终尺寸可读性，而不是只确认文件能打开。
...
| 数值验证、随机仿真、优化或长运行 | `../../../references/验证与运行治理.md` |
