---
name: LaTeX工具
description: 从官方或内置模板创建、编译和校验数学建模 LaTeX 论文项目，交付可复现的源码、PDF 与构建清单。
---
...
诊断会检查 `latexmk`、所选引擎、BibTeX/Biber、`pypdf`、`pdfimages` 和 `pdftoppm`。任一必需项缺失都先报告阻塞，不要写完整论文后才发现无法编译或校验。
