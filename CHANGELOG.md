...
## 2026-09-04：题意覆盖与验收证据

- 从原题核对量词、完整检查域、邻接关系与历史边界；分别检查漏约束和无依据的额外限制。
- M1 对可分离材料分两次提供输入，先记录原题解释再审作者模型；验证端独立重建薄弱约束范围。
- 可行性失败阻止相关推荐和收益结论通过，新增反例使相关旧验收失效。保留连续数值与决策空间规则。
- 移除不使用的 DeepSeek Harness 副本及同步脚本，后续只维护主 skill。
- 验证：旧版与候选1在11个小例中均判断正确，未测得准确率提升；最终调度修订另经分步审查补测，识别了遗漏窗口。有限测试不保证未来可靠性。
...
- 新增按需加载的 `references/可辨识性与复杂度控制.md`，覆盖反演、参数估计、标定和模型复杂度选择任务。
- 在模型合同中增加目标/辅助/固定/派生参数、观测依据、参数补偿风险、验证计划和结论边界，区分数据提供的信息与先验、正则化或外部标定提供的信息。
- 将 M1、P1、P2 分工补充为：M1 做静态辨识检查，P1 用已知真值小实例或解析特例检查恢复与失败识别，P2 检查验证信息隔离、复杂度升级依据、目标参数稳定性和不确定性条件。
- 修正编程手对病态矩阵的处理要求：区分数值尺度问题、结构不可辨识和数据支持不足；`lstsq`、`pinv` 或正则化只能改善计算或选择条件解，不能证明参数已被数据唯一确定。
- 将“避免直接套用简单模型”改为“简单基准优先”；新增机制或自由度必须对应可观察问题和验证证据，不以复杂度本身作为创新。
- 补充防止验证泄漏、相关数据分块、先验/正则化敏感性、预测可靠性与参数可靠性区分，以及模型变更后的阶段返修规则。
- 主版本与 dsh-plugin 知识库中对应的 Skill、角色说明和参考文档已同步；两份 Skill 入口通过 `quick_validate.py`，既有阶段协议测试 6 项通过，新增 18 处引用全部可解析。
